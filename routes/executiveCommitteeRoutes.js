...
const ExecutiveCommittee = mongoose.model('ExecutiveCommittee', executiveCommitteeSchema);
...
router.get('/:year/dummy', async (req, res) => {
    try {
        ...
        const dummy = [...response, ...response, ...response, ...response, ...response, ...response, ...response, ...response, ...response, ...response,];
